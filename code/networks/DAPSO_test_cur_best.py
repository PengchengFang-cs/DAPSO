...
def build_channel_basis(C: int, mode: str = 'identity', group: int | None = None) -> nn.Module:
    if mode == 'identity':
        return ChannelBasis(C, mode='identity')
    if group is None or group == C:
        return ChannelBasis(C, mode=mode)          # 全局 CxC
    return GroupedChannelBasis(C, group=group, mode=mode)

class GroupedChannelBasis(nn.Module):
    """
    按组做正交旋转：每组大小=group。group=None 或 group==C 时退化为全局 CxC。
    mode: 'identity' | 'learned_ortho'
    """
    def __init__(self, C: int, group: int | None = None, mode: str = 'identity'):
        super().__init__()
        self.mode = mode
        if mode == 'identity':
            self.group = C
            ...
            return
        group = C if group is None else int(group)
        assert C % group == 0, f"group({group}) must divide C({C})"
        self.group = group
        self.blocks = nn.ModuleList([ChannelBasis(group, mode=mode) for _ in range(C // group)])

    def forward(self, x, inverse: bool = False):  # x: (B,C,L)
        if self.mode == 'identity':
            return x
        xs = x.split(self.group, dim=1)
        ys = [blk(t, inverse=inverse) for blk, t in zip(self.blocks, xs)]
...
class DAPSO(nn.Module):
    ...
    def __init__(self, dim:int, rank:int=8, band:int=None, local:str="conv",
                 ...
                 h_gating_mode: str = "discrete", w_gating_mode: str = "discrete",
                 disc_bins: int | tuple = 96,
                 basis_h: str | None = None, basis_w: str | None = None,
                 group_h: int | None = None, group_w: int | None = None):
        super().__init__()
        self.C = dim
        self.R = rank
        self.band = band
        self.axis = axis
        self.h_gating_mode = h_gating_mode
        self.w_gating_mode = w_gating_mode
        ...
        # 通道谱基：HC / WC 各一套（若未指定，沿用 basis）
        bh = basis if basis_h is None else basis_h
        bw = basis if basis_w is None else basis_w
        self.UC_h = build_channel_basis(dim, mode=bh, group=group_h)
        self.UC_w = build_channel_basis(dim, mode=bw, group=group_w)
    ...
    def _gate_hc(self, H, C, device):
        if self.h_gating_mode == "continuous":
    ...
    def _gate_wc(self, W, C, device):
        if self.w_gating_mode == "continuous":
    ...
    def forward(self, x):
        ...
        elif self.axis == 'hc_hc':
            y = self._hc_branch(x); y = self._hc_branch(y)
        else:
