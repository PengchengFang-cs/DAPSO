...
from networks.fourier_conv import FourierLocalBlock
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        d_state: int = 16,
        model_type: str = 'dapso',   # kind: 'dapso', 'gfnet', 'afno', 'ffc', 'deformfno', 'fno', 'wfno', 'fsel', 'spectformer', 'identity'
        mixer_kwargs: dict = None,   # 传入各自的超参, 如 {'fft_norm':'backward'}
    ):
        super().__init__()
        ...
        mixer_kwargs = {} if mixer_kwargs is None else dict(mixer_kwargs)
        self.mixer = build_spectral_mixer(model_type, hidden_dim, **mixer_kwargs)

    def forward(self, input: torch.Tensor):
        # input: [B,H,W,C]
        x_attn = self.self_attention(self.ln_1(input))   # [B,H,W,C]
        x_nchw = x_attn.permute(0,3,1,2).contiguous()    # -> [B,C,H,W]

        # 统一在这里调用：DAPSO / GFNet / AFNO / FFC 都走这一行
        x_nchw = self.mixer(x_nchw)                      # 仍然 [B,C,H,W]（内部自残差）

        x_bhwc = x_nchw.permute(0,2,3,1).contiguous()    # -> [B,H,W,C]
        x = input + self.drop_path(x_bhwc)               # Block 外层残差
        return x
...
class VSSM_unrolled(nn.Module): 
    def __init__(self, patch_size=4, in_chans=2, num_classes=2, depths=[2, 2, 2, 2], 
                 ...
                 use_checkpoint=False, final_upsample="expand_first", model_type=None, **kwargs):
        super().__init__()
        print("OG MambaRecon")
