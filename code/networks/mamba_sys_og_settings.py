...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.dapso = build_dapso(hidden_dim)

        # self.fourier_conv = FourierLocalBlock(hidden_dim, hidden_dim)

    def forward(self, input: torch.Tensor):
        x_after  = self.self_attention(self.ln_1(input)).permute(0,3,1,2).contiguous() # [B,C,H,W]
        x_after  = self.dapso(x_after)
