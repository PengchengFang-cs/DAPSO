...
def main_worker(args):
    ...
    if args.model == "mamba_unrolled":
        ...
        from networks.convnext_tiny import ConvNeXtTinyRecon as VIM_seg
        print('ConvNeXtTiny selected.')
    elif args.model == "swin":
        from networks.swinunet_recon import SwinV2UNet as VIM_seg
        print('SwinV2UNet selected.')
    else:
    ...
    if args.model == "convnext":
        model = VIM_seg(in_chans=2).cuda(local_rank)
    elif args.model == "swin":
        model = VIM_seg().cuda(local_rank)
    else:
    ...
    #     model.parameters(),
    #     lr=args.base_lr,
    #     weight_decay=args.weight_decay   # 或直接填数字
    # )
    ...
    for epoch in range(start_epoch, max_epoch + 1):
        ...
        for i_batch, sampled_batch in enumerate(pbar):
            ...
            outputs, ba_loss = model(us_image, us_mask, coil_maps)
            outputs = torch.abs(outputs[:, 0, :, :] + 1j * outputs[:, 1, :, :])
            fs_target_abs = torch.abs(fs_target[:, 0, :, :] + 1j * fs_target[:, 1, :, :])
            
            loss = torch.mean(torch.abs(outputs - fs_target_abs)) + 1e-3 * ba_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--warmup_epochs', type=int, default=5, help='by-iter warmup epochs')
